	...
	while(1)
	{
		...
		if ((stat_btn & 0x1) != (stat_btn_pre & 0x1) && (stat_btn & 0x1) == 1){  					// Btn C pressed
			...
			// Function F13-20
				...
				trame = 0x1 + (xor_command<<1) + (command0<<10) + (led_setting1<<19) + 0x7FFE000000000;
			}

			// Send the trame
			// Trame_DCC <=  slv_reg1(18 downto 0) & slv_reg0;
			trame0 = trame & 0xFFFFFFFF;
			trame1 = (trame>>32) & 0x7FFFF;
			...
			IP_CENTRALE_DCC_mWriteReg(XPAR_IP_CENTRALE_DCC_0_S00_AXI_BASEADDR, IP_CENTRALE_DCC_S00_AXI_SLV_REG0_OFFSET, trame0);
			IP_CENTRALE_DCC_mWriteReg(XPAR_IP_CENTRALE_DCC_0_S00_AXI_BASEADDR, IP_CENTRALE_DCC_S00_AXI_SLV_REG1_OFFSET, trame1);
		}
